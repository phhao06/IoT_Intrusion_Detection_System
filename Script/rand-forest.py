...
filepath = "../Dataset/UNSW/UNSW-train_134.csv"
testpath = "../Dataset/UNSW/UNSW-NB15_train_2.csv"
...
test_dt = load_data(testpath)
test_dt.columns = features
print(len(test_dt.columns))
data = dt.replace({"proto": proto_num})
test_data = test_dt.replace({"proto": proto_num})

encode_label(data)
encode_label(test_data)

data = data.fillna(np.random.randint(100))
test_data = test_data.fillna(np.random.randint(100))
...
argus_fields_2 = [
    "sport", "dsport","dur", "proto", "state", "spkts", "dpkts", "sbytes", "dbytes", "sttl", "dttl", "sload", "dload", "sloss", "dloss", "swin", "dwin", "stcpb", "dtcpb", "tcprtt", "synack", "ackdat", "smeansz", "dmeansz",'stime', 'ltime',"label"
]
dataset =data[argus_fields_2]
testset = test_data[argus_fields_2]
x_train ,y_train = split_label_trainset(dataset)
x_test, y_test = split_label_trainset(testset)


print('The scikit-learn version is {}.'.format(sklearn.__version__))
clf = joblib.load("../Model/ids_rf_test_134.sav")
